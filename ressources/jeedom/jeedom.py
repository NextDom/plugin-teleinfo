...
class JeedomCom:
    ...
    def test(self):
        try:
            ...
            logging.error(
                'Callback result as a unknown error: %s. Please check your network configuration page' % str(e),)
            return False
        return True
...
class JeedomUtils:
    ...
    def set_log_level(level='error'):
        ...
        logging.basicConfig(level=JeedomUtils.convert_log_level(level), format=format, datefmt="%Y-%m-%d %H:%M:%S")

    @staticmethod
    def find_tty_usb(id_vendor, id_product, product=None):
        context = pyudev.Context()
        for device in context.list_devices(subsystem='tty'):
            if 'ID_VENDOR' not in device:
                continue
            if device['ID_VENDOR_ID'] != id_vendor:
                continue
            if device['ID_MODEL_ID'] != id_product:
                continue
    ...
    def byte_to_hex(byte_str):
        return ''.join(["%02X " % ord(x) for x in str(byte_str)]).strip()
...
class jeedom_socket_handler(StreamRequestHandler):
    def handle(self):
        ...
        logging.debug("Message read from socket: " + lg.strip().decode("UTF-8"))
